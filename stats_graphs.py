...
def generate_figures(filepath=default_path):
    f = h5.File(filepath, 'r')
    stats_dict, index_dict = get_dicts_for_graphs(f)
    f.close()
    ...
    all_stats.to_html("stats_table.html")  # create table of all stats for all plugins

    stats_dict, array_plugins = remove_arrays(stats_dict, index_dict)
    ...
    for row in ax:
        for axis in row:
            ...
            if i == 1:
                ...
                axis.text(maxx, maxy, p_legend, ha="left", va="top", bbox=dict(boxstyle="round", facecolor="red", alpha=0.4))
            stat.replace("_", " ")
            axis.set_title(stat)
            axis.grid(True)
            i += 1
    fig.suptitle("Projection Stats", fontsize="x-large")
    plt.savefig("projection_stats.png", bbox_inches="tight")
    ...
    for row in ax:
        for axis in row:
            ...
            if i == 1:
                ...
                axis.text(maxx, maxy, r_legend, ha="left", va="top", bbox=dict(boxstyle="round", facecolor="red", alpha=0.4))
            stat = stat.replace("_", " ")
            axis.set_title(stat)
            axis.grid(True)
            i += 1
...
def get_dicts_for_graphs(file):
    stats_dict = {}
    index_dict = {}
    stats_dict["projection"] = {"max": [], "min": [], "mean": [], "mean_std_dev": [],
                                "median_std_dev": [], "RMSD": []}
    stats_dict["reconstruction"] = {"max": [], "min": [], "mean": [], "mean_std_dev": [],
                                    "median_std_dev": [], "RMSD": []}
    index_dict["projection"] = {"max": [], "min": [], "mean": [], "mean_std_dev": [],
                                "median_std_dev": [], "RMSD": []}
    index_dict["reconstruction"] = {"max": [], "min": [], "mean": [], "mean_std_dev": [],
                                    "median_std_dev": [], "RMSD": []}
...
def remove_arrays(stats_dict, index_dict):
